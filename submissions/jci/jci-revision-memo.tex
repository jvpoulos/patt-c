...
\begin{itemize}
	\item Compare the PATT and PATT-C estimators against the SATE, which is just the ITT effect scaled by the compliance rate, since the SATE is a more appropriate benchmark
...
\subsubsection{Conditioning on $D_i$}
...
\subsection{Interpretation of PATT}\label{interpret}
...
R2 is correct in stating that the comparison between PATT and the \citet{Hartman} estimator is not an appropriate comparison. The (unadjusted ) PATT estimator in our study is the population-average causal effect of taking up treatment, adjusted according to the covariate distribution of RCT participants. In contrast, the \citet{Hartman} estimator is the ITT estimator reweighted according the to covariate distribution of the population. We clarify the distinction in Section \ref{sim} of the revised manuscript. We have also corrected the manuscript and to make clear that the response curve is fitted to treatment received $D_i$ rather than treatment eligibility $T_i$, since we cannot actually observe $T_i$ in the population. 

Assumption \ref{consistency} would indeed be violated if the \citet{Hartman} estimator were applied in a setting with noncompliance.
