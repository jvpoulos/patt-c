...
\begin{table}[h]
	...
	\begin{tabular}{@{}lll@{}}
		...
		SATE                                                 & 0.12 {[}-0.01, 0.04{]}          & -0.02 {[}-0.05, 0.008{]}    \\ \bottomrule
	\end{tabular}\\
\footnotesize{Notes: Sample Average Treatment Effect (SATE) is the ITT effect adjusted by the sample compliance rate in the RCT. Estimates in brackets represent 95\% bootstrap confidence intervals.}
\end{table}
...
Logistic regression (\texttt{LogicReg})  & 0.18 & 0 \\ 
Random forests, $\# preds. = 1$ (\texttt{randomForest}) & 0.25 & 0 \\ 
Random forests, $\# preds. = 10$ (\texttt{randomForest}) & 0.25 & 0 \\ 
Regularized linear regression, $\alpha=0.25$ (\texttt{glmnet})  & 0.18 & 0.45 \\ 
Regularized linear regression, $\alpha=0.5$ (\texttt{glmnet})  & 0.18 & 0 \\ 
   \hline
  \end{tabularx}
  \begin{tabularx}{\linewidth}{l*{3}{Y}}
    \toprule
...
Logistic regression (\texttt{LogicReg})  & 0.24 & 0 \\ 
Random forests, $\# preds. = 1$ (\texttt{randomForest}) & 0.39 & 0.42 \\ 
Random forests, $\# preds. = 10$ (\texttt{randomForest}) & 0.33 & 0 \\ 
Regularized linear regression, $\alpha=0.25$ (\texttt{glmnet})  & 0.24 & 0 \\ 
Regularized linear regression, $\alpha=0.5$ (\texttt{glmnet})  & 0.24 & 0 \\ 
   \hline
    \bottomrule
  \end{tabularx}
...
Logistic regression (\texttt{LogicReg})  & 0.18 & 0.68 \\ 
Random forests, $\# preds. = 1$ (\texttt{randomForest}) & 0.25 & 0 \\ 
Random forests, $\# preds. = 10$ (\texttt{randomForest}) & 0.24 & 0 \\ 
Regularized linear regression, $\alpha=0.25$ (\texttt{glmnet})  & 0.18 & 0.14 \\ 
Regularized linear regression, $\alpha=0.5$ (\texttt{glmnet})  & 0.18 & 0 \\  
...
Logistic regression (\texttt{LogicReg})  & 0.23 & 0 \\ 
Random forests, $\# preds. = 1$ (\texttt{randomForest}) & 0.39 & 0.25 \\ 
Random forests, $\# preds. = 10$ (\texttt{randomForest}) & 0.32 & 0.01 \\ 
Regularized linear regression, $\alpha=0.25$ (\texttt{glmnet})  & 0.23 & 0 \\ 
Regularized linear regression, $\alpha=0.5$ (\texttt{glmnet})  & 0.23 & 0 \\  
	\hline
	\bottomrule
\end{tabularx}
