...
%		\hline
		\textbf{Responses} &  $\mathbf{\bar{x}}$ & $\mathbf{sd}$& $\mathbf{\bar{x}}$ & $\mathbf{sd}$ & $\mathbf{\bar{x}}$ & $\mathbf{sd}$ \\  
	...
	\end{longtable}
\noindent
\footnotesize{Notes: weighted using OHIE and NHIS survey weights.}
\end{singlespace}
\restoregeometry
...
\begin{table}[h]
	...
	\begin{tabular}{@{}lccccc@{}}
		\toprule
		Outcome                & RCT complier & Adjusted population & Difference & Per-comparison & Family-wise\\
	...
	\end{tabular}\\ \\
\footnotesize{Notes: survey-weighted difference-in-means $p$-values calculated from two-sided t-test. Family-wise $p$-value calculated across the two different measures of health care utilization.}
\end{table}
