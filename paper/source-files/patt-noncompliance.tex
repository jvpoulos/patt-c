...
{\color{red}
In the assigned-treatment RCT group $(S_i = 1, T_i = 1)$, we train a gradient boosting algorithm on the covariates to predict who in the control group $(S_i = 1, T_i = 0)$ would comply with treatment ($C_i=1$), which is unobservable.} These individuals \textit{would have} complied had they been assigned to the treatment group. For this group of observed compliers to treatment and predicted compliers from the control group of the RCT, we estimate the response surface {\color{red}using gradient boosting} with features $(W^{1}_i, W^{2}_i, W^{3}_i)$ and $D_i$. The PATT-C is estimated according to the estimation procedure outlined above.
...
\includegraphics[width = 0.9\textwidth]{any-visit-plot}
    \caption{Comparison of estimators: any ER visit. Horizontal lines represent 95\% bootstrap confidence intervals constructed with 1,000 bootstrap samples.\label{fig:any-visit-plot}}
...
\begin{figure}[htbp]
\begin{center}
\includegraphics[width = 0.9\textwidth]{num-visits-plot}
    \caption{Comparison of estimators: $\#$ ER visits.\label{fig:num-visits-plot}}
%\vspace{10mm}
%\caption{\label{fig:num-visits-plot}}
%\vspace{10mm}
\end{center}
\end{figure}
...
\begin{figure}[htbp]
	\begin{center}
		...
		\caption{Comparison of estimators: $\#$ outpatient visits.\label{fig:num-out-plot}}
		%\vspace{10mm}
...
\begin{table}[h]
	...
	\begin{tabular}{@{}llll@{}}
		...
		SATE                                             & -0.001 [-0.02, 0.02]   & 0.005 [-0.04, 0.06]	& -0.02 [-0.19, 0.15]  \\ \bottomrule
	\end{tabular}\\
