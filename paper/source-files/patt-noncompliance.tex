...
This paper improves on the transportability of clinical trial results to a population by extending a method of estimating population average treatment effects to settings with noncompliance. Previous approaches to the problem of extrapolating RCT results to a population \citep{ImaKinStu08, stuart2011use, Hartman} are designed for settings where there is full compliance with treatment. This paper contributes to the literature by defining the assumptions required to identify complier--average causal effects for the target population and proposing an estimation procedure to recover this quantity.

\citet{Hartman} propose a method of reweighting the responses of individuals in an RCT according to the distribution of covariates in the target population in order to estimate \textcolor{red}{population average treatment effects}. We extend the method to estimate the complier--average causal effects for the target population from RCT data with noncompliance, and refer to this \textcolor{red}{estimand as the Population Average Treatment Effect on Treated Compliers (PATT-C)}. Noncompliance occurs when individuals who are assigned to the treatment group do not comply with the treatment; for individuals assigned to control, we are unable to observe who would have complied had they been assigned treatment. Noncompliance in treatment assignment is a prevalent issue in RCTs and biases the intention--to--treat (ITT) estimate towards zero.
...
\noindent \textcolor{red}{Restrictive exclusion criteria in RCTs can result in a sample covariate distribution that differs substantially from the population covariate distribution, thereby reducing the external validity of RCTs \citep{rothwell2005external}. High rates of exclusion also poses a threat to strong ignorability assumptions if exclusion increases the likelihood that there are unobserved differences between the RCT and target population that are correlated with potential outcomes. For example, the exclusion of patients at risk of complications from medical trials of potentially dangerous treatments would threaten strong ignorability if the known complication risk factors that constitute the exclusion criteria are correlated with unobservables (e.g., genetic information) that determine potential outcomes. The placebo tests described in Section \ref{placebo-tests} are designed to detect bias arising from violations of the strong ignorability assumptions.}\footnote{Note that \textcolor{red}{Assumptions \eqref{si_treat} and \eqref{si_ctrl}} also imply strong ignorability of sample assignment for treated and control noncompliers since we assume in that compliance is also independent of sample and treatment assignment, conditional on $W_i$ (Assumption \eqref{compl}). However, we are interested only on modeling the response surfaces for compliers.}
...
\tau_{\text{PATT}} = \ex\left( Y_{i01} - Y_{i00} \mid S_i=0\right).
\end{equation}
%
Under Assumptions \eqref{consistency}, \eqref{si_treat}, \eqref{si_ctrl}, and \eqref{ER}, Eq.~\eqref{tpatt} estimates the population-average causal effect of taking up treatment, adjusted according to the covariate distribution of population ``compliers.'' In contrast, the \citet{Hartman} population estimator is the ITT estimator reweighted according the to covariate distribution of the population.} Both PATT and PATT-C estimates are obtained by estimating the response surface on all individuals in the RCT and applying \ref{response-predict} of the estimation procedure to the population members.

\textcolor{red}{
We compare the population estimators to what we refer to as the Sample Average Treatment Effect on Treated Compliers (SATE-C), which is the ITT effect divided by the share of treated compliers in the RCT:
%	
\begin{equation}\label{tsatec}
\tau_{\text{SATE-C}} = \ex\left( Y_{i11} - Y_{i10} \mid S_i=1\right) / \pr(T_i = D_i = 1 \mid S_i=1).
\end{equation}	
%
Eq.~\eqref{tsatec} is identified under Assumptions \eqref{sutva}, \eqref{monotonicity}, and \eqref{ER}. \citet{freedman2006} points out that the instrumental variables estimator of the average causal effect for compliers proposed by \citet{Angrist1996} is a generalization of Eq.~\eqref{tsatec}.}
...
We set each individual's treatment received $D_i$ according to their treatment assignment and complier status and observe their responses $Y_{isd}$. In this design, the manner in which $S_i$, $T_i$, $D_i$, $C_i$, and $Y_{isd}$ are simulated ensures that Assumptions \eqref{consistency} -- \eqref{ER} hold.
...
\textcolor{red}{Within-household interference is ruled out in this RCT because treatment is randomized at the household level. Interference between households in a neighborhood threatens the no-interference assumption if, for example, the Medicaid coverage of individuals in treated households affects the health care use of individuals in households assigned to control \citep{hudgens2008toward}.}
...
\begin{figure}[htbp]
	\begin{center}
		...
		\caption{Simulated RMSE of PATT-C, PATT, and SATE-C, according to degree of confounding in compliance.\label{fig:rmse_boxplots_RateConC}}
...
\begin{figure}[htbp]
	\begin{center}
		...
		\caption{Simulated RMSE of PATT-C, PATT, and SATE-C, according to degree of confounding in treatment assignment.\label{fig:rmse_boxplots_RateConT}}
...
\begin{figure}[htbp]
	\begin{center}
		...
		\caption{Simulated RMSE of PATT-C, PATT, and SATE-C, according to degree of confounding in sample selection.\label{fig:rmse_boxplots_RateConS}}
...
\begin{table}[h]
	...
	\begin{tabular}{@{}llll@{}}
		...
		SATE-C                                             & -0.001 [-0.02, 0.02]   & 0.005 [-0.05, 0.06]	& -0.02 [-0.19, 0.14]  \\ \bottomrule
	\end{tabular}\\
