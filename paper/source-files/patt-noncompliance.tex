...
\citet{Hartman} propose a method of reweighting the responses of individuals in an RCT according to the distribution of covariates in the target population in order to estimate the population average treatment effect on the treated (PATT). We extend the method to estimate the complier--average causal effects for the target population from RCT data with noncompliance, and refer to this \textcolor{red}{estimand} as PATT-C. Noncompliance occurs when individuals who are assigned to the treatment group do not comply with the treatment; for individuals assigned to control, we are unable to observe who would have complied had they been assigned treatment. Noncompliance in treatment assignment is a prevalent issue in RCTs and biases the intention--to--treat (ITT) estimate towards zero.
...
causal effect for compliers.} Assumption \ref{monotonicity} ensures that crossover is only possible from treatment to control:
...
We conduct a simulation study to compare the performance of the PATT and PATT-C estimators. For comparison, we compare the population estimates to the \textcolor{red}{Sample Average Treatment Effect on Treated Compliers (SATE-C). SATE-C is estimated by scaling the ITT effect by the share of treated compliers in the RCT under the identifying Assumptions \ref{sutva}, \ref{monotonicity}, and \ref{ER}:
%	
\begin{equation}\label{tsatec}
\tau_{\text{SATE-C}} = \ex\left( Y_{i11} - Y_{i10} \mid S_i=1\right) / \pr(T_i = D_i = 1 \mid S_i=1).
\end{equation}	
%
\citet{freedman2006} points out that the instrumental variables estimator of the average causal effect for compliers proposed by \citet{Angrist1996} is a generalization of the SATE-C estimator.}
