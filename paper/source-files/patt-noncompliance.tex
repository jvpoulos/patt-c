...
\item {\color{red}For the observed compliers assigned to treatment and predicted compliers assigned to control, train a model to predict the response using $W_i$ and $D_i$, which gives $\ex(Y_{i1d} \mid S_i=1, C_i=1, D_i=d, W_i)$ for $d \in \{0,1\}$.}\label{response-model}
...
\tau_{\text{SATT-C}} = \ex(Y_{i11} \mid S_i=1, C_i=1, T_i=1) - \ex(Y_{i10} \mid S_i=1, C_i=1, T_i=1).
\end{equation}
\color{black}
