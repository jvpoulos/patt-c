source(paste0(repo.directory,"code/wtc.R")) 
...
lapply(y.col, function (i) WtC(Y.ohie[which(insurance.ohie==1),][[i]], 
                                      ...
                                      bootse=TRUE, # bootstrapped standard errors used to address heteroskadesticity issues when weights indicate probabilities of selection rather than the precision of estimates
                                      bootp = TRUE,
                                      bootn = 1000,
                                      ...
                                      samedata=FALSE,
                                      mean1=TRUE)) # weights should be forced to have an average value of 1